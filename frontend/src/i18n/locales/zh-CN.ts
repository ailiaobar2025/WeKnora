export default {
  ...
  embedPublish: {
    ...
    channelKeyHint: "渠道密钥出于安全考虑不会在列表中返回。若未保存，请点击重置获取新密钥（当前浏览器会话内会暂存）。",
    ...
    tokenHint: "渠道密钥出于安全考虑不会在列表中返回。复制浮窗脚本前，请在创建或重置时保存密钥（当前浏览器会话内会暂存）。",
    createdWithToken: "嵌入渠道已创建，新密钥已显示在输入框中，请立即复制并妥善保存",
    created: "嵌入渠道已创建",
    updated: "嵌入渠道已更新",
    saveFailed: "保存失败",
    deleted: "已删除",
    ...
    positionTopLeft: "左上角",
    publishToken: "渠道密钥",
    publishTokenHelp: "渠道密钥（em_…）是嵌入渠道的长期密钥，相当于 API Key。创建或重置时只显示一次，需写入第三方网站的浮窗脚本；列表接口不会返回，防止泄露。",
    ...
    sectionDeploy: "嵌入到网页",
    sectionWebhook: "事件回调",
    deployIntro: "保存设置后，按以下两步将聊天嵌入到您的网站。",
