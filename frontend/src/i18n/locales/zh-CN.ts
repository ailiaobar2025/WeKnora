export default {
  ...
  settings: {
    ...
    sandbox: {
      title: '沙箱后端',
      description: '管理智能体技能使用的沙箱运行环境，每个智能体可以选择一份空间后端配置。',
      editorDescription: '配置空间运行环境，Docker、本地进程、CubeSandbox 和 E2B 使用同一套管理流程。',
      stepConnection: '连接',
      stepTemplate: '模板',
      stepRuntime: '运行配置',
      ...
      confirmDelete: '确定删除沙箱后端「{name}」吗？',
      confirmDeleteWithAgents: '确定删除沙箱后端「{name}」吗？{agents}它们将在下次执行技能时报错。',
      deleteFailed: '删除失败',
      deleted: '已删除',
      forceDeleteTitle: '无法核实沙箱占用',
      forceDeleteConfirm: '无法连接该后端核实是否仍有沙箱。若该后端已废弃，可强制删除这份配置；若只是暂时不可达，强制删除会让残留沙箱无人回收。仍要强制删除吗？',
      forceDelete: '强制删除',
      disableScripts: '禁用沙盒执行',
      enableScripts: '启用沙盒执行',
      disableScriptsConfirm:
        '禁用后，本空间内所有智能体将无法在沙盒中执行技能脚本；仍可阅读技能内容。已在运行中的远端沙盒不会自动销毁，需结束或删除相关会话后才会释放。是否继续？',
      scriptsDisabled: '已禁用本空间沙盒执行',
      scriptsEnabled: '已恢复本空间沙盒执行',
      ...
      httpTimeoutHelp: '调用沙箱后端管理接口的等待上限，超过即视为端点不可用。留空按 30 秒。',
      sandboxTtl: '沙箱 TTL（秒）',
      ...
      deepCheck: '完整验证',
      recheck: '重新验证',
      deepCheckIntro: '保存前可以跑一次「完整验证」：真实创建一个临时沙箱、执行一次脚本并检查出网，随后自动销毁。',
      ...
      skipReasons: {
        ...
        environment_unavailable: '运行环境不可用，已跳过',
        sandbox_not_created: '沙箱未创建，已跳过',
        sandbox_exec_failed: '沙箱内执行失败，已跳过',
      },
      checks: {
        environment_available: '运行环境可用性',
        client_build: '客户端构建',
  ...
  agent: {
    ...
    editor: {
      ...
      sandboxBackend: '沙箱后端',
      sandboxBackendDefault: '不启用沙箱',
      sandboxBackendHint: '仅在启用技能执行时生效。所选配置被删除后，该智能体的技能执行会失败。',
      sandboxBackendMissing: '配置已删除',
      sandboxNoConfigs: '当前空间还没有沙箱后端配置，技能脚本不会执行。可在「设置 - 沙箱后端」中添加。',
      sandboxInfoTitle: '为什么要选后端？',
      sandboxInfoContent: '不同后端对应不同的账号、区域与配额。同一会话的沙箱一旦创建，后续的附件、产物与销毁都会锁定在创建时那份配置上，改这里只影响之后新建的沙箱。',
