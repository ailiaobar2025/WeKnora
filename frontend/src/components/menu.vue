                ...
                <t-tooltip :content="item.title" placement="right" :disabled="!uiStore.sidebarCollapsed">
                    <div @click="handleMenuClick(item.path)" @mouseenter="mouseenteMenu(item.path)"
                        ...
                        <div class="menu_item-box">
                            <div class="menu_icon">
                                <img class="icon"
                                    :src="getImgSrc(resolveMenuIcon(item))"
                                    alt="">
                            </div>
...
const isMenuItemActive = (itemPath: string): boolean => {
    ...
    switch (itemPath) {
        ...
        case 'tasks':
            return currentRoute === 'taskList' || currentRoute === 'taskDetail';
...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'workbench' || item.path === 'agents' || item.path === 'tasks' || item.path === 'knowledge-bases' || item.path === 'creatChat'
    );
});
...
const resolveMenuIcon = (item: MenuItem) => {
    if (item.icon === 'workbench') {
        return route.name === 'workbench' ? 'integration-green.svg' : 'integration.svg';
    }
    if (item.icon === 'task') {
        return route.name === 'taskList' || route.name === 'taskDetail' ? 'file-add-green.svg' : 'file-add.svg';
    }
    if (item.icon === 'zhishiku') return knowledgeIcon.value;
    if (item.icon === 'agent') return agentIcon.value;
    if (item.icon === 'organization') return organizationIcon.value;
    if (item.icon === 'logout') return logoutIcon.value;
    if (item.icon === 'setting') return settingIcon.value;
