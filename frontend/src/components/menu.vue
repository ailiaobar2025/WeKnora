...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'integrations' || item.path === 'organizations' || item.path === 'creatChat' || item.path === 'my-assistants' || item.path === 'customer-assistants' || item.path === 'trial-report' || item.path === 'quota-audit'
    );
});

const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        if (item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'integrations' || item.path === 'organizations' || item.path === 'creatChat' || item.path === 'my-assistants' || item.path === 'customer-assistants' || item.path === 'trial-report' || item.path === 'quota-audit') {
            return false;
        }
        return true;
    });
